	...
	, m_eState{ UNIT_STATE::NORMAL }
{
	...
	//Desc
	Ptr<CPrefab> prefab = CResMgr::GetInst()->FindRes<CPrefab>(L"CImageTitanPrefab");
...
void CTitanScript::begin()
{
	...
	Animator->CloneAnimation(L"Titan", *Animator);
...
void CTitanScript::SetDestPos(Vec3 _vPos)
{
	m_vTarget = _vPos;
	/*
	* 방향 벡터
...
void CTitanScript::JpsAlgorithm(Int32 x, Int32 z)
{
	Vec3 vPos = Transform()->GetRelativePos();
...
void CTitanScript::BeginOverlap(CCollider2D* _pOther)
{
}

void CTitanScript::Overlap(CCollider2D* _pOther)
{
...
void CTitanScript::EndOverlap(CCollider2D* _pOther)
{
}

void CTitanScript::SaveToFile(FILE* _File)
{
...
void CTitanScript::LoadFromFile(FILE* _File)
{
...
void CTitanScript::ProcessEnemy()
{
...
void CTitanScript::Move(Int32 _x, Int32 _z)
{
	m_pTargetObject = nullptr;
	m_bAttack = false;
	JpsAlgorithm(_x, _z);
...
void CTitanScript::SetMonsterHP()
{
...
void CTitanScript::PhaseEventOn()
{
	__super::PhaseEventOn();

	lstrcpy(CEngine::g_szFullName, L"Titan");
...
void CTitanScript::PhaseEventOff()
{
