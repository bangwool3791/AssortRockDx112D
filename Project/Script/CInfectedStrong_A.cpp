...
CInfectedStrong_A::CInfectedStrong_A()
...
void CInfectedStrong_A::begin()
{
	...
	Animator->CloneAnimation(L"InfectedStrong_A", *Animator);
...
void CInfectedStrong_A::SetDestPos(Vec3 _vPos)
{
	m_vTarget = _vPos;
	/*
	* 방향 벡터
	...
	*/
	...
	if (UNIT_STATE::NORMAL == m_eState)
		str = L"InfectedStrong_A_Normal";
	else if (UNIT_STATE::ATTACK == m_eState)
		str = L"InfectedStrong_A_Attack";
	else if (UNIT_STATE::RUN == m_eState)
		str = L"InfectedStrong_A_Walk";
	else if (UNIT_STATE::DEAD == m_eState)
	{
		str = L"InfectedStrong_A_Die_000";
		if (m_strBackUp != str)
...
void CInfectedStrong_A::JpsAlgorithm(Int32 x, Int32 z)
{
	Vec3 vPos = Transform()->GetRelativePos();
...
void CInfectedStrong_A::BeginOverlap(CCollider2D* _pOther)
{
}
...
void CInfectedStrong_A::SaveToFile(FILE* _File)
{
...
void CInfectedStrong_A::LoadFromFile(FILE* _File)
{
...
void CInfectedStrong_A::ProcessEnemy()
{
...
void CInfectedStrong_A::ChaseEnemy()
{
	vector<CGameObject*> vecEnemy = CLevelMgr::GetInst()->GetCurLevel()->GetLayer(2)->GetParentObjects();
...
void CInfectedStrong_A::Move(Int32 _x, Int32 _z)
{
	m_pTargetObject = nullptr;
	m_bAttack = false;
	JpsAlgorithm(_x, _z);
