...
CInfectedVenom::CInfectedVenom()
	:CScript{ INFECTEDVENOM }
	, m_fSpeed{ 100.f }
...
void CInfectedVenom::begin()
{
	...
	Animator->CloneAnimation(L"InfectedVenom", *Animator);
...
void CInfectedVenom::SetDestPos(Vec3 _vPos)
{
	m_vTarget = _vPos;
	/*
	* 방향 벡터
	...
	*/
	...
	if (UNIT_STATE::NORMAL == m_eState)
		str = L"InfectedVenom_Normal";
	else if (UNIT_STATE::ATTACK == m_eState)
		str = L"InfectedVenom_Attack";
	else if (UNIT_STATE::RUN == m_eState)
		str = L"InfectedVenom_Walk";
	else if (UNIT_STATE::DEAD == m_eState)
	{
		str = L"InfectedVenom_Die_000";
		if (m_strBackUp != str)
...
void CInfectedVenom::JpsAlgorithm(Int32 x, Int32 z)
{
	Vec3 vPos = Transform()->GetRelativePos();
...
void CInfectedVenom::BeginOverlap(CCollider2D* _pOther)
{
}
...
void CInfectedVenom::SaveToFile(FILE* _File)
{
...
void CInfectedVenom::LoadFromFile(FILE* _File)
{
...
void CInfectedVenom::Move(Int32 _x, Int32 _z)
{
	m_pTargetObject = nullptr;
	m_bAttack = false;
	JpsAlgorithm(_x, _z);
...
void CInfectedVenom::SetPlayerHP()
{
	if (m_pTargetObject)
	{
