...
CInfectedMedium_A::CInfectedMedium_A()
	:CScript{ INFECTEDMEDIUM_A }
	, m_fSpeed{ 100.f }
...
void CInfectedMedium_A::begin()
{
	...
	Animator->CloneAnimation(L"InfectedMedium_A", *Animator);
...
void CInfectedMedium_A::SetDestPos(Vec3 _vPos)
{
	m_vTarget = _vPos;
	/*
	* 방향 벡터
	...
	*/
	...
	if (UNIT_STATE::NORMAL == m_eState)
		str = L"InfectedMedium_A_Normal";
	else if (UNIT_STATE::ATTACK == m_eState)
		str = L"InfectedMedium_A_Attack";
	else if (UNIT_STATE::RUN == m_eState)
		str = L"InfectedMedium_A_Walk";
	else if (UNIT_STATE::DEAD == m_eState)
	{
		str = L"InfectedMedium_A_Die_000";
		if (m_strBackUp != str)
...
void CInfectedMedium_A::JpsAlgorithm(Int32 x, Int32 z)
{
	Vec3 vPos = Transform()->GetRelativePos();
...
void CInfectedMedium_A::BeginOverlap(CCollider2D* _pOther)
{
}

void CInfectedMedium_A::Overlap(CCollider2D* _pOther)
{
	//유닛 공통 영역
...
void CInfectedMedium_A::EndOverlap(CCollider2D* _pOther)
{
}

void CInfectedMedium_A::SaveToFile(FILE* _File)
{
...
void CInfectedMedium_A::LoadFromFile(FILE* _File)
{
...
void CInfectedMedium_A::ProcessEnemy()
{
...
void CInfectedMedium_A::ChaseEnemy()
{
	vector<CGameObject*> vecEnemy = CLevelMgr::GetInst()->GetCurLevel()->GetLayer(1)->GetParentObjects();
...
void CInfectedMedium_A::Move(Int32 _x, Int32 _z)
{
	m_pTargetObject = nullptr;
	m_bAttack = false;
	JpsAlgorithm(_x, _z);
...
void CInfectedMedium_A::SetPlayerHP()
{
	if (m_pTargetObject)
	{
